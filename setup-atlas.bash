...
# ------------------------------------------------------------------------------
echo -e "[ ${RED} Download lapack-3.5.0 ${NC} ]"
cd ${TMP_DIR}
test ! -f lapack.tgz && \
    wget http://www.netlib.org/lapack/lapack.tgz
tar xzf lapack.tgz

echo -e "[ ${RED} Configure lapack-3.5.0 ${NC} ]"
cd lapack-3.5.0
sed -e 's/^OPTS *=/OPTS\t= -O2 -fPIC -m64/' \
    -e 's/^NOOPT *=/NOOPT\t=-O0 -fPIC -m64 /' \
    make.inc.example > make.inc

echo -e "[ ${RED} Build lapack-3.5.0 ${NC} ]"
cd SRC
make -j ${NPROCS}

# ------------------------------------------------------------------------------
echo -e "[ ${RED} Download atlas-3.10.2 ${NC} ]"
cd ${TMP_DIR}
test ! -f atlas3.10.2.tar.bz2 && \
    wget http://downloads.sourceforge.net/project/math-atlas/Stable/3.10.2/atlas3.10.2.tar.bz2
tar xjf atlas3.10.2.tar.bz2

# ------------------------------------------------------------------------------
echo -e "[ ${RED} Configure atlas-3.10.2 ${NC} ]"
cd ATLAS/
...
../configure -b 64 -D c -DPentiumCPS=${CPUFREQ} \
    -Fa alg -fPIC \
    --with-netlib-lapack=${TMP_DIR}/lapack-3.5.0/lapack_LINUX.a

echo -e "[ ${RED} Build atlas-3.10.2 ${NC} ]"
...
# ------------------------------------------------------------------------------
echo -e "[ ${RED} Install atlas-3.10.2 ${NC} ]"
...
# ../configure -b 64 -D c -DPentiumCPS=2666 -Fa alg -fPIC --with-netlib-lapack=$HOME/src/lapack-3.5.0/lapack_LINUX.a
